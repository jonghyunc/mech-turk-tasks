...
if __name__ == '__main__':
    ...
    for j, assignment in enumerate(assignments_for_workerid):
        ...
        cv2.waitKey(1)
        print("%d: %s" % (j, assignment.AssignmentStatus))
        if str(assignment.AssignmentStatus) == 'Approved':
            nApproved += 1
        elif str(assignment.AssignmentStatus) == 'Rejected':
            nRejected += 1
            mturk.approve_rejected_assignment(str(assignment.AssignmentId))
        ...
        #         print("quit")
        #         sys.exit(1)
    print("nApproved: %d, nRejected: %d" % (nApproved, nRejected))
        # while (1):
        #     key = cv2.waitKey(0)
